...
export class PDIService {
  ...
  private irtProbability(
    theta: number,
    difficulty: number,
    discrimination = this.irtA,
  ): number {
  ...
  private updateAbility(
    ...
    for (const { correct, difficulty } of responses) {
      ...
      const info =
        this.irtProbability(theta, difficulty) *
        (1 - this.irtProbability(theta, difficulty));
  ...
  async submitQuizAttempt(userId: string, dto: SubmitQuizDto) {
    ...
    const skillScore = await this.prisma.skillScore.findUnique({
